...
interface Props {
  clientId: string;
  initialData: { name: string; phone: string; email?: string | null; notes?: string | null };
...
export default function EditClientButton({ clientId, initialData, settings }: Props) {
  ...
  const { register, handleSubmit, reset, formState: { errors, isSubmitting } } =
    useForm<FormData>({
      resolver: zodResolver(buildSchema(settings)),
      defaultValues: {
  ...
  async function onSubmit(data: FormData) {
    ...
    if (!res.ok) { setError(json.error?.message ?? g(settings, "error.updateClient", "Error al actualizar el cliente")); return; }
    reset();
    setOpen(false);
    router.refresh();
  }
  ...
  return (
    <>
      <button
        ...
        className="flex items-center gap-1 text-earth-muted hover:text-primary-dark transition p-1 rounded hover:bg-cream-dark"
        title={g(settings, "action.editClient", "Editar cliente")}
      >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-[#FFFDF5] rounded-2xl shadow-warm-lg w-full max-w-md mx-4 overflow-hidden z-10 border border-[#e6d5c3]">
            <div className="px-6 py-5 border-b border-[#e6d5c3] flex items-center justify-between">
              <div>
                <h2 className="text-lg font-bold text-earth">{g(settings, "form.title.editClient", "Editar cliente")}</h2>
                <p className="text-xs text-earth-muted mt-0.5">{g(settings, "form.subtitle.editClient", "Editá la información del cliente")}</p>
              </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="px-6 py-5 space-y-4">
              ...
              <div className="flex gap-3 pt-1">
                ...
                <button type="submit" disabled={isSubmitting} className="flex-1 px-4 py-2.5 text-sm font-bold bg-primary-dark hover:bg-primary-hover text-white rounded-lg shadow-warm-sm transition disabled:opacity-50">
                  {isSubmitting ? g(settings, "common.saving", "Guardando...") : g(settings, "action.updateClient", "Guardar cambios")}
                </button>
              </div>
            </form>
          </div>
